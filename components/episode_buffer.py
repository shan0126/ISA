...
class EpisodeBatch:
    def __init__(self,
        ...
        self.groups = groups                    # dict; groups.agents = n_agents
        self.batch_size = batch_size            # hyperparameter
        self.max_seq_length = max_seq_length    # env_info["episode_limit"] + 1
        self.preprocess = {} if preprocess is None else preprocess   # dict; preprocess.actions = (actions, list of one_hot)
        self.device = device 

        # 定义episode_batch中data的数据类型
        if data is not None:
            self.data = data
        else:
            self.data = SN()
