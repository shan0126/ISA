...
def my_main(_run, _config, _log):
    ...
    elif _config['alg'] in run_wo_goal:
        run_REGISTRY['run_wo_goal'](_run, config, _log)
    else:
...
def _get_config(params, arg_name, subfolder):
    ...
    if config_name is not None:
        with open(os.path.join(os.path.dirname(__file__), "config", subfolder, "{}.yaml".format(config_name)), "r") as f:
            try:
                # print(config_name)
                # print(os.path.join(os.path.dirname(__file__), "config", subfolder, "{}.yaml".format(config_name)))
                # exit(0)
...
if __name__ == '__main__':
    os.environ['SC2PATH'] = "/home/shuaih/StarCraftII"
    ...
    env_config = _get_config(params, "--env-config", "envs")   # load yaml config from /envs
    alg_config = _get_config(params, "--config", "algs")       # load yaml config from /algs
    ...
    map_name = parse_command(params, "env_args.map_name", config_dict['env_args']['map_name'])   # 3m
    algo_name = parse_command(params, "name", config_dict['name'])                               # qmix_env=8_adam_td_lambda
